...
export default function Quiz(props) {
    ...
    function selectAnswer(event, index) {
        setSelected(prevState => {
            return prevState.map((item, idx) => {
                return idx === index ? event.target.value : item
            })
        })
    }
    function checkAnswers() {
        ...
        for (let item of props.quizData) {
            let correctAn = item.correct_answer
            correctArr.push(correctAn)
        }
                
        let point = 0
        for (let i = 0; i < selected.length; i ++) {
            if (selected[i] === correctArr[i]) {
                point = point + 1
            }
        }
    ...
    const questionsElements = props.quizData.map((item, idx) => 
        <Question 
            key={idx}
    ...
    return(
        <div className="quiz-container">
            ...
            className="btn check-answer-btn">Check answers</button>:
            <div className="footer">
            <span className="score-text">You scored {score}/5 correct answers</span>
            <button
            onClick={newGame}
            className="btn check-answer-btn">play again</button>
            </div>}
